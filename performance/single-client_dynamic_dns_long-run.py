...
def change_access_point():
    ...
    user_domain = s.get_base_domain()
    current_ip = socket.gethostbyname(user_domain)

    # since we access the network from another point, we experience a delay towards the old edge
    os.system("sudo tc filter add dev ens3 parent 1: protocol ip prio 1 u32 match ip dst " + str(
        current_ip) + "/32 flowid 1:2")

    # obtain edge from ip
    edge_id = next(edge_name for edge_name, address in edge_ip_mapping.items() if address == current_ip)

    # migration request
    edge_node_list = ["edge1", "edge2", "edge3"]
    node_to_migrate = [x for x in edge_node_list if x != edge_id]
...
# parsing parameters
if len(sys.argv) != 5:
    print("you have to run: python3 script.py <handover_time_interval in min> <old_rtt_time> <run_index> <dns_ttl>")
    exit(1)

handover_time_interval = int(sys.argv[1])  # in minute
rtt_old_node = int(sys.argv[2])  # in ms
run_index = int(sys.argv[3])
dns_ttl = int(sys.argv[4])
...
# client setup
limits = httpx.Limits(max_keepalive_connections=None, max_connections=None, keepalive_expiry=0)
s = edge_http.Client("https://compute.lorenzogiorgi.com/orchestrator/login", "lorenzo", "lorenzo", False, verify=True,
...
df.to_csv(
    "./result/dynamic-dns"+str(dns_ttl)+"-" + str(handover_time_interval) + "-" + str(rtt_old_node) + "-" + str(run_index) + ".csv")
